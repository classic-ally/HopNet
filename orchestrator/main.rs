...
async fn create_mesh(docker: &Docker, mesh_id: u32, node_count: u32) -> Result<()> {
    ...
    match create_hopnet_network(docker, &network_name).await {
        Ok(network_id) => {
            ...
            if let Some((container_name, _container_id, ip_address)) = containers.first() {
                ...
                if let Err(e) = setup_node_0(&container_name, &ip_address).await {
                    ...
                    for (name, container_id, _) in containers {
                        ...
                        let _ = docker.stop_container(&container_id, None::<bollard::container::StopContainerOptions>).await;
                ...
                if containers.len() > 1 {
                    ...
                    for (node_index, (container_name, _container_id, node_ip)) in containers.iter().enumerate().skip(1) {
                        ...
                        if let Err(e) = register_node_with_node_0(node_0_ip, node_id, container_name, node_ip).await {
...
async fn create_hopnet_container(docker: &Docker, container_name: &str, network_name: &str) -> Result<(String, String)> {
    ...
    let config = ContainerCreateBody {
        ...
        host_config: Some(HostConfig {
            // No port bindings - container only accessible via network
            ..Default::default()
        }),
        ..Default::default()
    };
...
async fn setup_node_0(node_name: &str, ip_address: &str) -> Result<()> {
    let client = reqwest::Client::new();
    let url = format!("http://{}:34633/setup", ip_address);
...
async fn get_jwt_token(node_ip: &str) -> Result<String> {
    let client = reqwest::Client::new();
    let login_url = format!("http://{}:34633/login", node_ip);
...
async fn register_node_with_node_0(node_0_ip: &str, node_id: u32, node_name: &str, node_ip: &str) -> Result<()> {
    let client = reqwest::Client::new();
    
    // Step 1: Get the public key from the node's /setup GET route
    let get_setup_url = format!("http://{}:34633/setup", node_ip);
    ...
    let jwt_token = get_jwt_token(node_0_ip).await?;
    
    // Step 3: Register the node with node 0 via POST /nodes
    let register_url = format!("http://{}:34633/nodes", node_0_ip);
    let node_data = json!({
        "node_id": node_id,
        "name": node_name,
        "ip_address": node_ip,
            ...
            .await
        {
            Ok(response) => {
                ...
                if status == reqwest::StatusCode::CREATED {
                    ...
                    if start_time.elapsed() + retry_interval > timeout_duration {
                        return Err(anyhow::anyhow!("Node registration failed with status: {}", status));
                    }
                } else {
                    println!("  Unexpected status: {} - {}", status, response_text);
                    if start_time.elapsed() + retry_interval > timeout_duration {
                        return Err(anyhow::anyhow!("Node registration returned unexpected status: {}", status));
                    }
                }
            }
            Err(e) => {
    ...
    if !containers.is_empty() {
        ...
        for container in containers {
            if let Some(ref id) = container.id {
                ...
                if let Err(e) = docker.stop_container(id, None::<bollard::container::StopContainerOptions>).await {
                    println!("    Warning: Failed to stop container {}: {}", name, e);
                }
                
                println!("  Removing container: {}", name);
                if let Err(e) = docker.remove_container(id, None::<bollard::container::RemoveContainerOptions>).await {
...
async fn show_mesh_status(docker: &Docker, mesh_id: u32) -> Result<()> {
    ...
    for (node_id, container) in node_data {
        ...
        // Query node status
        let status = get_node_status(node_id, &ip_address).await;
...
async fn get_node_status(node_id: u32, ip_address: &str) -> Result<NodeStatus> {
    let client = reqwest::Client::new();
    
    // First get JWT token
    let jwt_token = match get_jwt_token(ip_address).await {
        Ok(token) => token,
        Err(_) => return Err(anyhow::anyhow!("Auth failed")),
    };
    
    // Query consensus state with JWT
    let consensus_url = format!("http://{}:34633/consensus", ip_address);
