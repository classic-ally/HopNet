...
use crate::tests::files::{
    upload_file, wait_for_fragment_distribution, get_fragment_distribution,
};
...
impl TestScenario for FragmentHealthCheck {
    ...
    async fn run(&self, _mesh_id: u32, nodes: &[NodeInfo], _flags: &[String]) -> Result<TestResult> {
        ...
        let distribution = match wait_for_fragment_distribution(&nodes[0], &full_path, Duration::from_secs(30)).await {
            Ok(dist) => {
                print_and_add_check(&mut result, Check {
        ...
        let test_fragment = match distribution.fragments.iter()
            .find(|f| !f.nodes_with_fragment.is_empty())
        {
            ...
            None => {
                print_and_add_check(&mut result, Check {
                    ...
                    passed: false,
                    detail: Some("Cannot test health checks without distributed fragments".to_string()),
                });
                result.duration = start.elapsed();
                return Ok(result);
            }
                    ...
                    match resp.json::<FragmentHealthCheckResponse>().await {
                        Ok(health) => {
                            ...
                            for peer_result in &health.results {
                                let peer_should_have = expected_node_ids.contains(&peer_result.node_id);
                            ...
                            print_and_add_check(&mut result, Check {
                                ...
                                detail: Some(format!(
                                    ...
                                    health.healthy, health.unhealthy, health.errors, health.total_nodes
                                )),
                            });
                        }
                        Err(e) => {
