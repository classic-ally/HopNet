...
impl TestScenario for DeviceTokenSessionBootstrap {
    ...
    async fn run(&self, _mesh_id: u32, nodes: &[NodeInfo], _flags: &[String]) -> Result<TestResult> {
        ...
        if !poll_until_fp_enumerate_works(&client, nodes, &register_response.api_key, PROPAGATION_TIMEOUT).await {
            print_and_add_check(&mut result, Check {
                name: "Device token propagation timeout".to_string(),
                passed: false,
                detail: Some(format!("Not all nodes accepted token within {}s", PROPAGATION_TIMEOUT.as_secs())),
            });
            result.duration = start.elapsed();
            return Ok(result);
        }
        ...
        match dp_upload_file(&client, &nodes[upload_node], &register_response.api_key, "root", &test_filename, test_content.as_bytes()).await {
            Ok(_) => {
                print_and_add_check(&mut result, Check {
                    name: format!("Upload '{}' on node {} via DocumentProvider", test_filename, upload_node),
                    passed: true,
        ...
        let file_id = match poll_for_dp_item_on_all_nodes(&client, nodes, &register_response.api_key, None, &test_filename, PROPAGATION_TIMEOUT).await {
            Some(id) => {
                print_and_add_check(&mut result, Check {
                    name: format!("File '{}' replicated to all {} nodes", test_filename, nodes.len()),
                    passed: true,
            ...
            None => {
                print_and_add_check(&mut result, Check {
                    ...
                    passed: false,
                    detail: Some(format!("File did not appear on all nodes within {}s", PROPAGATION_TIMEOUT.as_secs())),
                });
                result.duration = start.elapsed();
                return Ok(result);
            }
        ...
        match dp_download_file(&client, &nodes[download_node], &register_response.api_key, &file_id).await {
            Ok(downloaded) => {
                let content_matches = downloaded == test_content.as_bytes();
                print_and_add_check(&mut result, Check {
                    name: format!("Download file from node {} and verify content", download_node),
                    passed: content_matches,
                    detail: if content_matches {
                        Some(format!("{} bytes, content matches", downloaded.len()))
                    } else {
                        Some(format!("Content mismatch: expected {} bytes, got {}", test_content.len(), downloaded.len()))
                    },
                });
                if !content_matches {
                    result.duration = start.elapsed();
        ...
        if poll_until_token_rejected(&client, nodes, &register_response.api_key, PROPAGATION_TIMEOUT).await {
            print_and_add_check(&mut result, Check {
            ...
            print_and_add_check(&mut result, Check {
                ...
                passed: false,
                detail: Some(format!("Not all nodes rejected token within {}s", PROPAGATION_TIMEOUT.as_secs())),
            });
        }
...
impl TestScenario for FileProviderDeviceTokenAuth {
    ...
    async fn run(&self, _mesh_id: u32, nodes: &[NodeInfo], _flags: &[String]) -> Result<TestResult> {
        ...
        match fp_create_folder(&client, &nodes[0], &register_response.api_key, "NSFileProviderRootContainerItemIdentifier", &folder_name).await {
            Ok(_) => {
                print_and_add_check(&mut result, Check {
        ...
        let folder_id = match poll_for_fp_item_on_all_nodes(&client, nodes, &register_response.api_key, None, &folder_name, PROPAGATION_TIMEOUT).await {
            Some(id) => {
                print_and_add_check(&mut result, Check {
                    name: format!("Folder '{}' found on all {} nodes", folder_name, nodes.len()),
                    passed: true,
            ...
            None => {
                print_and_add_check(&mut result, Check {
                    ...
                    passed: false,
                    detail: Some(format!("Folder did not appear on all nodes within {}s", PROPAGATION_TIMEOUT.as_secs())),
                });
                result.duration = start.elapsed();
                return Ok(result);
            }
        ...
        match fp_create_file(&client, &nodes[0], &register_response.api_key, &folder_id, &test_filename, test_content.as_bytes()).await {
            Ok(_) => {
                print_and_add_check(&mut result, Check {
        ...
        let file_identifier = match poll_for_fp_item_on_all_nodes(&client, nodes, &register_response.api_key, Some(&folder_id), &test_filename, PROPAGATION_TIMEOUT).await {
            Some(id) => {
                print_and_add_check(&mut result, Check {
                    name: format!("File '{}' found on all {} nodes", test_filename, nodes.len()),
                    passed: true,
                    detail: Some(format!("identifier: {}", id)),
                });
                id
        ...
        let download_node = if nodes.len() > 1 { 1 } else { 0 };
        match fp_download(&client, &nodes[download_node], &register_response.api_key, &file_identifier).await {
            Ok(downloaded) => {
                let content_matches = downloaded == test_content.as_bytes();
                print_and_add_check(&mut result, Check {
        ...
        match fp_changes(&client, &nodes[0], &register_response.api_key, 0).await {
            Ok(resp) => {
                ...
                print_and_add_check(&mut result, Check {
                    ...
                    passed: changes_ok,
                    detail: Some(format!("{} items returned, folder={}, file={}", resp.items.len(), has_folder, has_file)),
                });
            }
            Err(e) => {
                print_and_add_check(&mut result, Check {
        ...
        if poll_until_token_rejected(&client, nodes, &register_response.api_key, PROPAGATION_TIMEOUT).await {
            print_and_add_check(&mut result, Check {
                name: format!("All {} nodes reject revoked token on FP endpoints", nodes.len()),
                passed: true,
                detail: None,
            });
        } else {
...
async fn fp_enumerate(client: &Client, node: &NodeInfo, api_key: &str, parent_item_identifier: Option<&str>) -> Result<FPEnumerateResponse> {
    let mut url = format!("http://{}:{}/integrations/fileprovider/enumerate", node.ip_address, node.port);
...
async fn fp_create_folder(client: &Client, node: &NodeInfo, api_key: &str, parent_id: &str, folder_name: &str) -> Result<()> {
    let url = format!("http://{}:{}/integrations/fileprovider/create", node.ip_address, node.port);
...
async fn fp_create_file(client: &Client, node: &NodeInfo, api_key: &str, parent_id: &str, filename: &str, content: &[u8]) -> Result<()> {
    let url = format!("http://{}:{}/integrations/fileprovider/create", node.ip_address, node.port);
...
async fn fp_download(client: &Client, node: &NodeInfo, api_key: &str, identifier: &str) -> Result<Vec<u8>> {
    let url = format!(
...
async fn fp_changes(client: &Client, node: &NodeInfo, api_key: &str, since_height: i32) -> Result<FPChangesResponse> {
    let url = format!(
...
async fn dp_upload_file(client: &Client, node: &NodeInfo, api_key: &str, parent_id: &str, filename: &str, content: &[u8]) -> Result<()> {
...
async fn dp_download_file(client: &Client, node: &NodeInfo, api_key: &str, file_id: &str) -> Result<Vec<u8>> {
    let url = format!(
...
async fn dp_enumerate(client: &Client, node: &NodeInfo, api_key: &str, parent_id: Option<&str>) -> Result<Vec<DocumentProviderItem>> {
...
async fn poll_until_fp_enumerate_works(client: &Client, nodes: &[NodeInfo], api_key: &str, timeout: Duration) -> bool {
...
async fn poll_until_token_rejected(client: &Client, nodes: &[NodeInfo], api_key: &str, timeout: Duration) -> bool {
    let deadline = Instant::now() + timeout;
    while Instant::now() < deadline {
        let mut all_rejected = true;
        for node in nodes {
            let url = format!("http://{}:{}/integrations/fileprovider/enumerate", node.ip_address, node.port);
            match client
                .get(&url)
                .header("Authorization", format!("Bearer {}", api_key))
                .timeout(Duration::from_secs(10))
