...
use crate::tests::files::{
    upload_file, download_file, download_file_headers, download_file_range,
};
...
impl TestScenario for RangeDownload {
    ...
    async fn run(&self, _mesh_id: u32, nodes: &[NodeInfo], flags: &[String]) -> Result<TestResult> {
        ...
        print_and_add_check(&mut result, Check {
            ...
            passed: true,
            detail: Some(format!("{} ({}B), {} ({}B)", png_filename, png_size, txt_filename, txt_size)),
        });
        ...
        let (full_png, png_headers) = download_file_headers(&nodes[0], &format!("/{}", png_filename)).await?;
        ...
        print_and_add_check(&mut result, Check {
            ...
            detail: Some(format!("expected: {}, got: {:?}", png_size, png_headers.content_length)),
        });
        ...
        let (_, txt_headers) = download_file_headers(&nodes[0], &format!("/{}", txt_filename)).await?;
        // text/plain may include charset
        let txt_type_ok = txt_headers.content_type.as_ref().map(|t| t.starts_with("text/plain")).unwrap_or(false);
        print_and_add_check(&mut result, Check {
        ...
        let (range_bytes, range_headers) = download_file_range(&nodes[0], &format!("/{}", txt_filename), 0, Some(99)).await?;
        ...
        let content_range_ok = range_headers.content_range.as_deref() == Some(&expected_content_range);
        print_and_add_check(&mut result, Check {
            name: "Content-Range header correct".to_string(),
            passed: content_range_ok,
            detail: Some(format!("expected: {}, got: {:?}", expected_content_range, range_headers.content_range)),
        });
        ...
        let (tail_bytes, tail_headers) = download_file_range(&nodes[0], &format!("/{}", txt_filename), tail_start, None).await?;
        ...
        print_and_add_check(&mut result, Check {
            ...
            detail: Some(format!("got {} bytes, expected {}", tail_bytes.len(), expected_tail.len())),
        });

        // --- Check 6: Range beyond file size returns 416 ---
        let huge_start = txt_size as u64 + 1000;
        let (_, bad_headers) = download_file_range(&nodes[0], &format!("/{}", txt_filename), huge_start, None).await?;
        ...
        if flags.contains(&"multi-chunk".to_string()) {
            ...
            let big_reached = wait_for_minimum_view(nodes, big_target, Duration::from_secs(120)).await?;
            ...
            if big_reached {
                ...
                let (span_bytes, span_headers) = download_file_range(
                    &nodes[0], &format!("/{}", big_filename), span_start, Some(span_end),
                ).await?;

                let span_ok = span_headers.status == 206 && span_bytes == &big_contents[span_start as usize..=span_end as usize];
                print_and_add_check(&mut result, Check {
                    name: "Multi-chunk range spanning boundary".to_string(),
                    passed: span_ok,
                    detail: Some(format!("status: {}, bytes: {} (expected {})",
                        span_headers.status, span_bytes.len(), span_end - span_start + 1)),
                });
            } else {
                print_and_add_check(&mut result, Check {
