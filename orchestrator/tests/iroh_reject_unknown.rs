    ...
    async fn run(&self, _mesh_id: u32, nodes: &[NodeInfo], _flags: &[String]) -> Result<TestResult> {
        ...
        for mesh_node in &mesh_nodes {
            ...
            let target_node_id = iroh::PublicKey::from_bytes(
                &pubkey_bytes.try_into().map_err(|_| anyhow::anyhow!("Invalid pubkey length"))?
            )?;
            ...
            let leaked_ips: Vec<String> = remote_info.as_ref()
                .map(|info| {
                    info.addrs()
            ...
            print_and_add_check(&mut result, Check {
                ...
                detail: Some(if ip_leak_passed {
                    match &remote_info {
                        Some(info) => {
                            let relay_count = info.addrs().filter(|a| a.addr().is_relay()).count();
                            if relay_count > 0 {
                                format!("Only relay addr (no direct IPs) - {} relay(s)", relay_count)
                            } else {
                    ...
                    format!("LEAKED {} direct IP(s): {}", leaked_ips.len(), leaked_ips.join(", "))
                }),
            });
        }
