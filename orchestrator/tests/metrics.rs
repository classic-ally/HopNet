...
use crate::tests::{
    Check, NodeInfo, TestResult, TestScenario, get_max_view, print_and_add_check,
...
impl TestScenario for MetricsCollection {
    ...
    async fn run(
        ...
        let response = client
            ...
            .header(
                "Authorization",
                format!("Bearer {}", trigger_node.jwt_token),
            )
        ...
        let trigger_resp = match response {
            Ok(resp) if resp.status().is_success() => match resp.json::<TriggerResponse>().await {
                Ok(data) => {
                    print_and_add_check(
                        &mut result,
                        Check {
        ...
        let propagated =
            wait_for_minimum_view(nodes, initial_view + 1, Duration::from_secs(10)).await?;
        print_and_add_check(
            &mut result,
            Check {
                name: "Consensus propagation".to_string(),
                passed: propagated,
                detail: Some(format!("Waited for view >= {}", initial_view + 1)),
            },
        );
        ...
        for node in nodes {
            let url = format!("https://{}:{}/api/metrics", node.ip_address, node.port);
            match client
                .get(&url)
