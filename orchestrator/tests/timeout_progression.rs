...
impl TestScenario for TimeoutProgression {
    ...
    async fn run(&self, mesh_id: u32, nodes: &[NodeInfo], _flags: &[String]) -> Result<TestResult> {
            ...
            None => {
                print_and_add_check(
                    ...
                    Check {
                        ...
                        passed: false,
                        detail: Some(format!(
                            "Leader node {} not found in {} nodes",
                            leader_node_id,
                            nodes.len()
                        )),
                    },
        ...
        let surviving_nodes: Vec<NodeInfo> = nodes
            .iter()
            .enumerate()
        ...
        for node in &surviving_nodes {
            match get_consensus_history(node).await {
                Ok(history) => {
                    let tc_for_view = history
                        .iter()
                        .find(|entry| entry.view == initial_view as i64 && entry.has_tc);
            ...
            Ok(false) => {
                print_and_add_check(
                    ...
                    Check {
                        ...
                        passed: false,
                        detail: Some(format!(
                            "Node {} did not respond within 30s",
                            leader_node_id
                        )),
                    },
                );
            ...
            Ok(false) => {
                print_and_add_check(
                    ...
                    Check {
                        ...
                        passed: false,
                        detail: Some(format!(
                            "node {} did not reach view {} in 90s",
                            leader_node_id, target_view
                        )),
                    },
                );
        ...
        match get_consensus_history(&restarted_node).await {
            Ok(history) => {
                let has_tc = history
                    .iter()
                    .any(|entry| entry.view == initial_view as i64 && entry.has_tc);
                print_and_add_check(
                    &mut result,
                    Check {
