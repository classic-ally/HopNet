...
async fn fetch_db_stats(node: &NodeInfo) -> Result<DbStatsClient> {
    ...
    Err(last_err.unwrap_or_else(|| anyhow::anyhow!("GET /debug/db-stats failed with no error captured")))
}
...
fn diff_phase(name: &str, wall: Duration, attempted: u64, succeeded: u64, before: &DbStatsClient, after: &DbStatsClient) -> PhaseSummary {
    let wall_secs = wall.as_secs_f64();
    let count_in_phase = after.commit_latency_us.count.saturating_sub(before.commit_latency_us.count);
    PhaseSummary {
        name: name.to_string(),
        wall_secs,
        ops_attempted: attempted,
        ops_succeeded: succeeded,
        ops_per_sec: if wall_secs > 0.0 { succeeded as f64 / wall_secs } else { 0.0 },
        delta_commits: after.counters.txn_commits.saturating_sub(before.counters.txn_commits),
        delta_rollbacks: after.counters.txn_rollbacks.saturating_sub(before.counters.txn_rollbacks),
...
impl TestScenario for DbPragmaBench {
    ...
    async fn run(&self, _mesh_id: u32, nodes: &[NodeInfo], flags: &[String]) -> Result<TestResult> {
        ...
        for f in flags {
            if let Some(v) = f.strip_prefix("write=") { if let Ok(n) = v.parse() { write_count = n; } }
            if let Some(v) = f.strip_prefix("writers=") { if let Ok(n) = v.parse() { writers = n; } }
            if let Some(v) = f.strip_prefix("readers=") { if let Ok(n) = v.parse() { readers = n; } }
            if let Some(v) = f.strip_prefix("read=") { if let Ok(n) = v.parse() { read_count = n; } }
            if let Some(v) = f.strip_prefix("mixed_secs=") { if let Ok(n) = v.parse() { mixed_secs = n; } }
            if let Some(v) = f.strip_prefix("payload_bytes=") { if let Ok(n) = v.parse() { payload_bytes = n; } }
        }
        ...
        for h in a_handles { let _ = h.await; }
        ...
        let summary_a = diff_phase("write-burst", phase_a_start.elapsed(), a_total_attempted, a_succeeded, &baseline, &after_a);
        ...
        let summary_b = diff_phase("read-burst", phase_b_start.elapsed(), read_count as u64, b_succeeded, &after_a, &after_b);
        ...
        let total_ops_c = (writes_done.load(Ordering::Relaxed) + reads_done.load(Ordering::Relaxed)) as u64;
        let summary_c = diff_phase("mixed-sustained", phase_c_start.elapsed(), total_ops_c, total_ops_c, &after_b, &after_c);
        ...
        match serde_json::to_string_pretty(&report).map(|s| std::fs::write(&report_path, s)) {
            Ok(Ok(())) => print_and_add_check(&mut result, Check {
                name: "Wrote JSON report".to_string(),
                passed: true,
                detail: Some(report_path),
            }),
            Ok(Err(e)) => print_and_add_check(&mut result, Check {
