...
use crate::NodeInfo;
use crate::tests::files::{download_file, list_files, upload_file};
...
impl TestScenario for MultiUserIsolation {
    ...
    async fn run(&self, mesh_id: u32, nodes: &[NodeInfo], _flags: &[String]) -> Result<TestResult> {
                        ...
                        if idx == 0 {
                            print_and_add_check(
                                ...
                                Check {
                                    ...
                                    passed: true,
                                    detail: Some(format!(
                                        "Then logged in on {} remaining nodes",
                                        nodes.len() - 1
                                    )),
                                },
                            );
        ...
        let view_after_bob_upload =
            get_max_view(nodes).await.unwrap_or(view_after_owner_upload) + 1;
                ...
                match try_download_file(bob_node, "/owner-secret.txt").await {
                    ...
                    Ok(Err(status)) => {
                        ...
                        print_and_add_check(
                            ...
                            Check {
                                name: format!(
                                    "Bob got unexpected status {} for owner file on node {}",
                                    status, i
                                ),
                                passed: false,
                    ...
                    Ok(Ok(_)) => {
                        print_and_add_check(
                            ...
                            Check {
                                name: format!(
                                    "Bob downloaded owner's file on node {} (isolation breach!)",
                                    i
                                ),
                                passed: false,
                                detail: None,
                ...
                match try_download_file(node, "/bob-file.txt").await {
                    ...
                    Ok(Err(status)) => {
                        print_and_add_check(
                            ...
                            Check {
                                name: format!(
                                    "Owner got unexpected status {} for Bob's file on node {}",
                                    status, i
                                ),
                                passed: false,
                                detail: Some("Expected 404".to_string()),
                    ...
                    Ok(Ok(_)) => {
                        print_and_add_check(
                            ...
                            Check {
                                name: format!(
                                    "Owner downloaded Bob's file on node {} (isolation breach!)",
                                    i
                                ),
                                passed: false,
                                detail: None,
