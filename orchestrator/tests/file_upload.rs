...
use crate::tests::files::{
    download_file_from_all_nodes, list_files_from_all_nodes, upload_file, verify_all_identical,
    verify_listings_identical,
};
...
impl TestScenario for FileUploadConsistency {
    ...
    async fn run(
        ...
        let test_contents =
            format!("HopNet consistency test file created at {}", timestamp).into_bytes();
        ...
        if let Some(files_array) = listings[0].as_array() {
            let file_found = files_array
                .iter()
                .any(|file| file["path"].as_str() == Some(&full_path));
