...
async fn test_file_size(
    ...
    println!("  Testing {}MB file ({} bytes)...", size_mb, test_contents.len());
    ...
    match wait_for_minimum_view(nodes, target_view, consensus_timeout).await {
        Ok(true) => {
            print_and_add_check(result, Check {
                name: format!("{}MB: Consensus propagated to view {}", size_mb, target_view),
                passed: true,
    ...
    let downloads = match download_file_from_all_nodes_with_timeout(nodes, &full_path, download_timeout).await {
        Ok(data) => {
            print_and_add_check(result, Check {
    ...
    if downloads[0] == test_contents {
        ...
        print_and_add_check(result, Check {
            ...
            passed: false,
            detail: Some(format!("Expected {} bytes, got {} bytes", test_contents.len(), downloads[0].len())),
        });
        return Ok(false);
    }
