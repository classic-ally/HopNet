...
impl TestScenario for FragmentDistribution {
    ...
    async fn run(&self, _mesh_id: u32, nodes: &[NodeInfo], _flags: &[String]) -> Result<TestResult> {
        ...
        let test_contents = format!("HopNet fragment distribution test file created at {}", timestamp)
            .into_bytes();
        ...
        let distribution = match wait_for_fragment_distribution(&nodes[0], &full_path, distribution_timeout).await {
            Ok(dist) => {
                print_and_add_check(&mut result, Check {
                    ...
                    detail: Some(format!(
                        ...
                        dist.fragment_count, dist.original_count, dist.recovery_count, dist.placement_height
                    )),
                });
                dist
            }
        ...
        let downloads = match download_file_from_all_nodes_with_timeout(nodes, &full_path, Duration::from_secs(15)).await {
            Ok(data) => {
                print_and_add_check(&mut result, Check {
                    ...
                    passed: true,
                    detail: Some(format!("Retrieved {} downloads using distributed fragments", data.len())),
                });
                data
            }
            Err(e) => {
        ...
        match verify_all_identical(&downloads) {
            Ok(_) => {
                print_and_add_check(&mut result, Check {
                    name: "All nodes return identical file content (distributed retrieval)".to_string(),
                    passed: true,
