...
import app.hopnet.storage.data.ContentStorage
import app.hopnet.storage.data.DocumentRepository
import app.hopnet.storage.data.HopNetDocument
import app.hopnet.storage.data.LogRepository
import app.hopnet.storage.data.LoggedDocument
import app.hopnet.storage.data.LoggedRoot
import app.hopnet.storage.data.toLogged
 ...
 */
class HopNetDocumentsProvider : DocumentsProvider() {

    companion object {
        private const val TAG = "HopNetDocumentsProvider"
     ...
     * A root represents a top-level storage location (like "HopNet" in the Files app sidebar).
     */
    ...
    override fun queryDocument(documentId: String, projection: Array<out String>?): Cursor {
        ...
        if (documentId == HopNetDocument.ROOT_ID) {
            ...
            result.newRow().apply {
                ...
                add(Document.COLUMN_DISPLAY_NAME, "HopNet")
    ...
    override fun queryChildDocuments(
        ...
        result.setNotificationUri(context?.contentResolver,
            android.provider.DocumentsContract.buildChildDocumentsUri(
                "app.hopnet.storage", parentDocumentId
            )
        )
    ...
    override fun renameDocument(documentId: String, displayName: String): String {
        ...
        if (renamed.parentId != null) {
            context?.contentResolver?.notifyChange(
                android.provider.DocumentsContract.buildChildDocumentsUri(
                    "app.hopnet.storage", renamed.parentId
                ), null
            )
        }
