...
pub fn capture(
    ...
    // get_folder_contents — returns FileProviderEnumerateResult (not Serialize)
    {
        ...
        results.insert(
            "db::fileprovider::get_folder_contents(user=0,path=/root)".into(),
            {
                match fileprovider::get_folder_contents(
                    pool.get(),
                    0,
                    &parent_pattern,
                    siv_key,
                    ...
                    Ok(result) => {
                        ...
                        items.sort_by(|a, b| {
                            a.identifier
                                .cmp(&b.identifier)
                                .then(a.modification_height.cmp(&b.modification_height))
                        });
    ...
    {
        results.insert(
            "db::fileprovider::get_folder_changes_since_height(user=0,height=0)".into(),
            {
                match fileprovider::get_folder_changes_since_height(
                    pool.get(),
                    0,
                    encrypted_root,
                    0,
    ...
    if let Some(data_id) = ctx.data_block_ids.first() {
        results.insert(
            "db::fileprovider::get_file_path_by_data_id".into(),
            wrap(|| fileprovider::get_file_path_by_data_id(pool.get(), data_id.clone(), 0)),
        );
    }

    // get_inode_id_by_path
    results.insert(
        "db::fileprovider::get_inode_id_by_path(path=/root)".into(),
        wrap(|| fileprovider::get_inode_id_by_path(pool.get(), encrypted_root, 0)),
    );

    // is_folder_empty
    results.insert(
        "db::fileprovider::is_folder_empty(path=/root)".into(),
        wrap(|| fileprovider::is_folder_empty(pool.get(), encrypted_root, 0)),
    );
}
