...
pub fn capture(pool: &Pool<SqliteConnectionManager>, ctx: &FixtureContext, results: &mut BTreeMap<String, FunctionResult>) {
    ...
    results.insert("db::files::get_files(user=0,path=/root)".into(), wrap(|| {
        files::get_files(pool.get(), encrypted_root.clone(), 0, siv_key, siv_nonce)
            .map(|mut items| { items.sort_by(|a, b| a.path.cmp(&b.path)); items })
    }));

    results.insert("db::files::get_local_fragment_count".into(), wrap(|| {
        files::get_local_fragment_count(pool.get())
    }));

    // get_file_access for first data block, user 0
    if let Some(db_id) = ctx.data_block_ids.first() {
        let conn = pool.get().unwrap();
        results.insert("db::files::get_file_access(user=0)".into(), wrap(|| {
            files::get_file_access(&conn, db_id, 0)
        }));
    }
}
