...
pub fn capture(pool: &Pool<SqliteConnectionManager>, results: &mut BTreeMap<String, FunctionResult>) {
    ...
    results.insert("db::metrics::get_metric".into(), wrap(|| {
        metrics::get_metric(pool.get())
    }));

    results.insert("db::metrics::get_nodes_to_measure(exclude=0)".into(), wrap(|| {
        metrics::get_nodes_to_measure(pool.get(), 0)
    }));
}
...
pub fn capture_time_windowed(pool: &Pool<SqliteConnectionManager>, results: &mut BTreeMap<String, FunctionResult>) {
    ...
    results.insert("db::metrics::get_all_node_metrics(height=5)".into(), wrap(|| {
        metrics::get_all_node_metrics(pool.get(), 5)
    }));
}
