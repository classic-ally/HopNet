...
pub fn capture(pool: &Pool<SqliteConnectionManager>, ctx: &FixtureContext, results: &mut BTreeMap<String, FunctionResult>) {
    ...
    for node_id in 0..3i32 {
        let key = format!("db::inventory::compute_inventory_differential(node={})", node_id);
        results.insert(key, {
            match inventory::compute_inventory_differential(pool.get(), node_id) {
                Ok(mut diff) => {
                    ...
                    diff.fragments_added.sort_by(|a, b| a.0.as_bytes().cmp(b.0.as_bytes()));
                    diff.fragments_removed.sort_by(|a, b| a.0.as_bytes().cmp(b.0.as_bytes()));
                    FunctionResult::Ok {
    ...
    if !ctx.fragment_hashes.is_empty() {
        results.insert("db::inventory::batch_query_fragment_inventory".into(), wrap(|| {
