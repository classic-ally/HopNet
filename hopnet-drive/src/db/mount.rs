...
fn row_to_item(
    ...
    let height: Option<u64> = row.get::<_, Option<i64>>(7)?.map(hopnet_common::height::height_from_db);
...
pub fn changes_since(
    ...
    let rows = stmt
        .query_map(params![user_id, hopnet_common::height::height_to_db(since_height)], |row| {
            ...
            let height: Option<u64> = row.get::<_, Option<i64>>(8)?.map(hopnet_common::height::height_from_db);
