//! Drive share DB operations (RFC-015).
//!
//! Moved verbatim from the host's `db::shares`; the host re-exports this
//! module at its old path. Queries JOIN the host-owned `users` table —
//! drive SQL may READ host tables (same SQLite DB; the ownership boundary
//! is code, not schema).
