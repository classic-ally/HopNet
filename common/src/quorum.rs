...
//!
//! The pure arithmetic surface of the consensus quorum policy — shared by the
//! consensus engine (which pairs it with malachite `ThresholdParams` via the
//! `thresholds_for` extension trait in `hopnet-consensus`) and by the storage
//! durability watermark (which sizes its reserve off the same fault budget).
//! Keeping it here, in the malachite-free common crate, makes the two systems
//! a single source of truth: neither can drift from the other because both
//! call `QuorumProfile::quorum`.
//!
...
impl QuorumProfile {
    ...
    /// source of truth for the membership policy's headroom/parity math AND
    /// the storage durability fault budget `B(v) = v − quorum(v)`.
...
mod tests {
    ...
    // Impact: the whole S6 threshold selection and the storage fault budget.
    #[test]
    fn auto_composite() {
