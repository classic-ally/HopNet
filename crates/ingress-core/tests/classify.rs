...
/// bytes per resource type, then persist the capsule. Sizes are made to match
...
// capsule without moving a single byte, and stamp asset_modified_at so the
// next delivery is NoOp.
// Should: rewrite the capsule (favorite reflected) then stamp the date.
...
// the recomposed publish document back to the two-resource shape.
#[tokio::test]
async fn revert_deletes_rows_and_reaps_blobs() {
    ...
    assert_eq!(
        doc.resources.len(),
        2,
        "recomposed document reflects the reverted shape"
    );
}
...
async fn tombstoned_photo_redelivered_classifies_restore() {
    ...
    assert!(
        store
            ...
            .descriptor_json
            .is_some(),
        "capsule still present after the restore"
    );
}
