<script lang="ts">
  ...
  const TRI_ROWS: { key: 'live' | 'raw' | 'favorite'; label: string; icon: string }[] = [
    { key: 'live', label: 'Live Photo', icon: 'i-carbon-motion' },
    { key: 'raw', label: 'RAW', icon: 'i-carbon-raw' },
    { key: 'favorite', label: 'Favorite', icon: 'i-carbon-favorite' },
  ];
  ...
  function setTri(key: 'live' | 'raw' | 'favorite', v: TriState) {
