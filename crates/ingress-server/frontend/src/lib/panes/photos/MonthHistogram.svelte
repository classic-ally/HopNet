<script lang="ts">
  ...
  // strip of bars; hovering expands the rail (the parent flex layout shrinks
  // the grid beside it — width transition, grid keeps its scroll position).
  let {
    buckets,
    onJump,
  }: {
    buckets: MonthBucket[];
    /** Optional: click a month to jump the grid to it (future slot). */
  ...
  >
    {#each buckets as bucket, i (bucket.month)}
      ...
      >
        ...
        {/if}
        <span class="bar bg-mauve/70" style={`width: ${Math.max(8, (bucket.count / max) * 100)}%`}
        ></span>
      </button>
    {/each}
