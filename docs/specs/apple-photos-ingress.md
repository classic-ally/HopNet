...
- Remaining [ ]: deploy on thor (nix-config derivation, reverse-proxy, host secret) — redeploy needed for the filter/histogram routes; embed `frontend/dist` in the binary.