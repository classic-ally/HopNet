...
- Provisioning travels through the keychain service `com.hopnet.desktop.photo-ingress`: the enable route mints the device token (`api_key` + `base_url`) and stores the library provisioning (`blob_root`, optional `sidecar_root_remote`); at startup the daemon auto-binds the personal library from those values via the ensure-only `ensure_personal_library` FFI (a deliberate narrow reversal of Phase 6's "libconfig is CLI-only" — the sandboxed GUI cannot write the data dir, so the daemon is the only capable process; bind/rename/set-retention stay CLI-only). Token minting is enablement-gated: login only self-heals an existing provision, and setup mints nothing.
...
- **Resolve pre-pass (consensus photo identity)**: each pass opens with one batched `POST /api/photos/client/resolve` carrying the claimed photos' `cloud_id`s. The node (which holds the user key at device-auth time; the daemon holds none) returns per cloud_id the keyed **cloud fingerprint** (RFC-011 §Cloud Fingerprint), any already-committed consensus photo id, and this device's **responsibility standing**:
  ...
  - *Responsibility gate*: if this device is not the holder (`other` | `unclaimed`), remaining photos are HELD under a distinct `parked_responsibility` state — edge-logged (`publish_not_responsible` / `responsibility_regained`), zero attempts consumed. Claims are explicit and JWT-only (`POST /api/photos/ingress/claim`; the device transaction route rejects the claim tx kind and 403s non-holder mutations as the admission backstop — the fingerprint UNIQUE pair is the correctness backstop behind that). A daemon never claims for itself; the enablement UI (or curl) designates deliberately.
...
- **Driver exit codes** (`ingress-publish-e2e publish`): 0 drained, 2 unreachable-park, 3 responsibility-park.
