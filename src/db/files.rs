...
pub fn insert_files(
    ...
    match db_connection {
        Ok(mut db_lock) => {
            ...
            let current_height = crate::db::consensus::get_current_consensus_height(&tx)?;
            ...
            for inode in inodes {
                ...
                log_modification(&tx, inode_id, owner_id, None, None, Some(&inode.path), current_height)?;
            }
...
pub fn modify_item(
    ...
    match db_connection {
        Ok(mut db_lock) => {
            ...
            tracing::debug!("modify_item: Querying inodes table for inode_id={} user_id={} execute={}", inode_id, user_id, execute);
            let item_info: Option<(hopnet_common::InodeType, String)> = tx.query_row(
            ...
            let old_parent_id = get_parent_id(&tx, &current_encrypted_path, user_id)
                .unwrap_or_else(|e| {
            ...
            log_modification(&tx, inode_id.clone(), user_id, old_parent_id, old_path_ref, new_path_ref, current_height)?;
            
            if execute {
                tx.commit().map_err(|_| DatabaseError::ProcessingError)?;
                tracing::debug!("modify_item: Transaction committed successfully for inode_id={}", inode_id);
            } else {
                tx.rollback().map_err(|_| DatabaseError::ProcessingError)?;
                tracing::debug!("modify_item: Transaction rolled back (dry run) for inode_id={}", inode_id);
            }
