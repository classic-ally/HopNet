...
/// Map the projection's embedded DataRecord onto the substrate's
/// BlobInsertOp (the DRIVE envelope reshape at C2 makes this the wire shape).
fn blob_op_from_data_record(
    data_record: &crate::db::DataRecord,
) -> hopnet_storage::store::BlobInsertOp {
    hopnet_storage::store::BlobInsertOp {
        blob_id: data_record.id.clone(),
        integrity_hash: data_record.data.hash,
        added_bytes: data_record.data.added_bytes,
        file_size: data_record.file_size,
        fragments: data_record
            .data
            .fragments
            .iter()
            .map(|f| hopnet_storage::store::FragmentMeta {
                blob_id: f.data_block_id.clone(),
                chunk_number: f.chunk_number,
                local_index: f.local_index,
                fragment_id: f.fragment_id.clone(),
                fragment_hash: f.fragment_hash,
                recovery: matches!(f.chunk_type, crate::db::ChunkType::Recovery),
            })
            .collect(),
        access: data_record
            .file_access_entries
    ...
    for inode in inodes {
        ...
        let data_id: Option<CustomUUID> = match inode.data_id {
            ...
            Some(either::Either::Right(data_record)) => {
                // Substrate half: blob + fragments (stored_locally probed
                // in-crate) + wraps, via the crate apply (RFC-014).
                let op = blob_op_from_data_record(&data_record);
...
pub fn modify_item(
    ...
    if let (Some(new_data_id), Some(data_record)) = (new_data_block_id, new_data_record) {
        ...
        // Substrate half: blob + fragments (stored_locally probed in-crate)
        // + wraps, via the crate apply (RFC-014).
        let op = blob_op_from_data_record(&data_record);
        hopnet_storage::store::apply_blob_insert(
            db_tx,
            &op,
        ...
        .map_err(|e| {
            tracing::error!("modify_item: apply_blob_insert failed: id={} error={e}", op.blob_id);
            DatabaseError::InsertError
        })?;
