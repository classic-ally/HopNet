...
pub async fn handle_metrics_collection(
    ...
    match collect_all_node_metrics(app_state, measurement_timeout).await {
        Ok(metrics) => {
            ...
            tracing::info!(
                "Collected {} metrics, submitting to consensus",
                metrics.len()
            );

            // Serialize metrics for consensus submission using existing pattern
            let serialized_metrics =
                bincode::serde::encode_to_vec(&metrics, bincode::config::standard()).map_err(
                    |e| {
            ...
            let source_node_id = app_state.get_node_id().map_err(|_| {
                Error::Failed(Arc::new(Box::new(std::io::Error::new(
                    std::io::ErrorKind::InvalidInput,
