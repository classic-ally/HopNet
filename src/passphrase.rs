...
/// Generate an 8-word passphrase from the EFF large wordlist.
/// 8 words × log2(7776) ≈ 103.4 bits of classical entropy.
pub fn generate_passphrase() -> String {
    let mut rng = rand::rng();
    (0..8)
        .map(|_| EFF_LARGE_WORDLIST[rng.random_range(0..7776)])
        .collect::<Vec<&str>>()
        .join(" ")
}

/// Normalize a passphrase for comparison: lowercase, collapse whitespace, trim.
...
mod tests {
    ...
    fn test_generate_passphrase_word_count() {
        ...
        assert_eq!(words.len(), 8, "Passphrase should have exactly 8 words");
    }

    #[test]
    fn test_generate_passphrase_all_words_in_wordlist() {
        let passphrase = generate_passphrase();
        for word in passphrase.split_whitespace() {
            assert!(
                EFF_LARGE_WORDLIST.contains(&word),
                "Word '{}' should be in the EFF wordlist",
                word
            );
        }
    }
    ...
    #[test]
    fn test_normalize_passphrase() {
        let original = "  Alpha   BRAVO  charlie  ";
        let normalized = normalize_passphrase(original);
        assert_eq!(normalized, "alpha bravo charlie");
    }
    ...
    #[test]
    fn test_normalize_passphrase_with_uppercase_and_spaces() {
        let passphrase = generate_passphrase();
