...
#[cfg(target_os = "macos")]
use objc2_file_provider::{NSFileProviderManager, NSFileProviderDomain, NSFileProviderItemIdentifier, NSFileProviderRootContainerItemIdentifier, NSFileProviderWorkingSetContainerItemIdentifier};
#[cfg(target_os = "macos")]
...
#[cfg(target_os = "macos")]
pub async fn register_fileprovider_domain(username: &str) -> Result<(), Box<dyn std::error::Error>> {
    info!("Registering HopNet FileProvider domain with macOS for user: {}", username);
    ...
    let completion_handler = RcBlock::new(move |error: *mut objc2_foundation::NSError| {
        if error.is_null() {
            ...
            info!("HopNet should now appear in Finder sidebar as '{}'s HopNet'", username_for_closure);
            ...
            if let Some(manager) = unsafe { objc2_file_provider::NSFileProviderManager::managerForDomain(&temp_domain) } {
            ...
            unsafe {
                ...
                match code {
                    ...
                    -1000 => {
                        error!("💡 Error -1000 usually means the extension bundle is missing or malformed");
                    }
                    _ => {
                        error!("💡 Unknown FileProvider error. Check system logs for more details.");
                    }
                }
            }
        }
...
pub async fn unregister_fileprovider_domain() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let completion_handler = RcBlock::new(|error: *mut objc2_foundation::NSError| {
        if error.is_null() {
            ...
            unsafe {
                let error_ref = &*error;
                warn!("Failed to remove HopNet FileProvider domain: {:?}", error_ref);
            }
        }
    });
...
#[cfg(target_os = "macos")]
pub async fn signal_fileprovider_refresh(test_mode: bool) -> Result<(), Box<dyn std::error::Error>> {
    ...
    tracing::debug!("Domain values: identifier={:?}, display_name={:?}", 
                   domain_identifier, display_name);
    ...
    unsafe {
        ...
        tracing::debug!("Created domain object: identifier={:?}, display_name={:?}", 
                       actual_identifier, actual_display_name);
    }
    ...
    if manager.is_none() {
        tracing::debug!("NSFileProviderManager::managerForDomain returned None for domain identifier: {:?}", 
                       domain_identifier);
    ...
    tracing::debug!("Signaling working set container: {:?}", working_set_identifier);
    ...
    let completion_handler = RcBlock::new(|error: *mut objc2_foundation::NSError| {
        if error.is_null() {
            ...
            unsafe {
                let error_ref = &*error;
                tracing::debug!("signalEnumeratorForContainerItemIdentifier completion: FAILED - {:?}", error_ref);
                warn!("Failed to signal FileProvider incremental sync: {:?}", error_ref);
            }
        }
    });
    
    // Signal the working set enumerator to trigger incremental sync
    tracing::debug!("Calling signalEnumeratorForContainerItemIdentifier with manager={:?}, working_set={:?}", 
                   manager, working_set_identifier);
    unsafe {
        manager.signalEnumeratorForContainerItemIdentifier_completionHandler(&working_set_identifier, &completion_handler);
    }
...
#[cfg(target_os = "macos")]
pub async fn initialize_fileprovider_on_startup(app_state: &AppState) -> Result<(), Box<dyn std::error::Error>> {
