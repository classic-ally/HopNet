#[cfg(target_os = "macos")]
use super::keychain::{KeychainEnvironment, FileProviderConfig, store_config, load_config, remove_config};
use uuid::Uuid;
...
async fn test_keychain_setup() {
    ...
    // Load it back
    let loaded_config = load_config(KeychainEnvironment::Test)
        .expect("Failed to load test config from keychain");
    
    // Verify it matches our randomly generated values (not any stale data)
    assert_eq!(loaded_config.api_key, random_api_key, "API key should match the randomly generated value");
    assert_eq!(loaded_config.base_url, test_base_url, "Base URL should match the randomly generated value");
